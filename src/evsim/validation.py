...
def run_core_validation(seed: int = 7) -> dict[str, Any]:
    ...
    checks.append(
        Check("static_scene_no_events", len(events) == 0, {"events": len(events)})
    )
    ...
    checks.append(
        Check("bright_ramp_multiple_positive", positive, {"events": len(events)})
    )

    events = _stream(_ideal_config(), [white, black])
    negative = bool(len(events) > 1 and np.all(events["polarity"] == -1))
    checks.append(
        Check("dark_ramp_multiple_negative", negative, {"events": len(events)})
    )
    ...
    actual = _stream(
        analytic_cfg,
        [np.full((1, 1), 5, dtype=np.uint8), np.full((1, 1), 250, dtype=np.uint8)],
    )
...
def benchmark_backends(
    ...
    rng = np.random.default_rng(seed)
    images = [
        rng.integers(0, 256, (height, width), dtype=np.uint8) for _ in range(frames)
    ]
    results: dict[str, Any] = {}
    for backend in ("vectorized", "loop"):
        config = _ideal_config(
            **{"simulation.backend": backend, "runtime.progress": False}
        )
        sim = EventSimulator(config)
